...
export function getAllPostsForContentType(contentType: string, tag?: string): Post[] {
  const allPostsData: Post[] = [];
  const typeDirectory = path.join(contentDirectory, contentType);
  
  if (!fs.existsSync(typeDirectory)) return [];

  // Check if _index.mdx exists and read its frontmatter for show_dates
  const indexPath = path.join(typeDirectory, '_index.mdx');
  let showDates = true; // Default to showing dates
  ...
  if (fs.existsSync(indexPath)) {
    try {
      ...
      console.error(`Error reading _index.mdx for ${contentType}:`, error);
    }
  }
...
export function getPostBySlug(slug: string, contentType?: string): Post | null {
  try {
    // If contentType is specified, use getAllPostsForContentType to get all posts
    // regardless of show_in_recent setting
    if (contentType) {
    ...
    // If no contentType specified, fall back to getAllPosts which respects show_in_recent
